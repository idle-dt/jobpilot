...
def get_connection(
    ...
    ``busy_timeout_ms`` bounds how long a query waits on a locked database before
    raising ``OperationalError``; it defaults to ``settings.db_busy_timeout_ms``.
    Passed via sqlite3's ``timeout`` parameter (seconds) rather than a ``PRAGMA``
    so no value is interpolated into SQL.
    """
    timeout_ms = settings.db_busy_timeout_ms if busy_timeout_ms is None else busy_timeout_ms
    db_path.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(
        str(db_path), check_same_thread=False, timeout=timeout_ms / 1000
    )
