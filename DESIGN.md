...
All colors are defined as CSS custom properties under `:root` / `[data-theme="light"]` and overridden under `[data-theme="dark"]`. Components must use `var(--name)` — never hardcoded hex values.

### Dark Theme Badge/Label Pattern
Light theme badges use `rgba(0, 0, 0, 0.06)` backgrounds. These are invisible on dark surfaces. Every badge/label variant needs a dark override using white-based alpha:
```css
[data-theme="dark"] .badge {
    background: rgba(255, 255, 255, 0.10);
}
```
Follow the same pattern already established for `.signal` elements.

### Theme Toggle
The theme toggle button uses inline SVG icons (sun/moon) with `stroke="var(--ink)"` for automatic theme color adaptation. Icon visibility is controlled purely via CSS selectors (`[data-theme="dark"] .icon-sun`, `[data-theme="light"] .icon-moon`) — no JavaScript toggling needed. This prevents flash-of-wrong-icon on page load.

### Toolbar Card (`.toolbar-card`)
A lifted pill strip (`--lifted` background, `--radius-pill`, `--shadow-1`) that sits above the "Review Queue" heading. Breaks out ~32px wider than the main container via negative margins (narrower than nav, wider than job cards). Contains:
- **Left**: stats count (`15 emails + 34 scraped jobs`), vertical separator, green sync dot, last sync timestamp
- **Right**: ink-black Sync pill button with refresh SVG icon; replaced by a progress bar during sync

The toolbar card uses `visibility: hidden` (not `display: none`) on the sync button during sync to prevent height jumping. The progress bar and button overlap via CSS Grid (`grid-area: 1 / 1`) in `.toolbar-right-group`.

### Heading Row (`.heading-row`)
Flex row with `justify-content: space-between` and `align-items: baseline`. Title (`h2`) left, sort controls right. Replaces the old `.page-heading` layout.

### Sync Button & Progress
The sync button (`button.sync-btn`) is an ink-black pill with a refresh SVG icon. Uses `!important` overrides to fully neutralize Pico CSS button defaults. During sync:
- Dot turns orange with pulsing animation (`.sync-dot.syncing`)
...
### Job Card Layout (`.card-top` / `.card-main` / `.card-aside`)
Two-column layout inside `.row-card`:
- **Left (`.card-main`)**: title first, company/location below, then badge row (source, score, details fetched)
- **Right (`.card-aside`)**: date and Active/Expired toggle
- On mobile (≤768px): stacks vertically, aside becomes a horizontal row

### Active/Expired Toggle
The expired badge (`.expired-toggle`) is a clickable `<span>` with `role="button"` that toggles via htmx POST. Styled with a visible border (`1.5px solid`) to distinguish it from static badges. Active state uses `--dust` border; expired uses a faint red border. Both darken on hover to reinforce interactivity.

### Anti-Patterns (Dark Theme)
- Never use `rgba(0, 0, 0, ...)` for badge/label backgrounds without a `[data-theme="dark"]` override
- Never use `currentColor` for SVG strokes inside buttons — Pico CSS overrides button color. Use `var(--ink)` directly
- Never use `opacity: 0` to hide elements that should not occupy layout space — use `display: none`
- Never use Unicode text characters for icons — use inline SVG for pixel-perfect centering
